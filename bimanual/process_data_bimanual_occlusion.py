...
sys.path.append("/home/baothach/dvrk_shape_servo/src/dvrk_env/dvrk_gazebo_control/src/bimanual/occlusion")
...
data_processed_path = f"/home/baothach/shape_servo_data/rotation_extension/bimanual/multi_{args.obj_category}/processed_data_occlusion"
...
with torch.no_grad():
    for i in range(start_index, max_len_data):
        ...
        mp_pos_1 = np.array([mani_point_1[0,3], mani_point_1[1,3]- two_robot_offset, mani_point_1[2,3] + ROBOT_Z_OFFSET])
        ...
        num_occlusion = np.random.randint(1,5)  # 1-4 occlusion points
        occluded_pc = add_occlusion(pc.transpose(1,0), num_occlusion=num_occlusion, nn_radius=[0.02, 0.04], vis=True)
        modified_pc = np.vstack([down_sampling(occluded_pc).transpose(1,0), mp_channel_1, mp_channel_2])# pc with 4th and 5th channel with value of 1 if near the MP point and 0 elsewhere
