...
class SingleBoxSDFDataset(Dataset):
    """Shape servo dataset."""
    '''
    ...
    def __init__(self, percentage = 1.0):
        ...
        self.dataset_path = "/home/baothach/shape_servo_data/RL_shapeservo/box/embeddings_data/"
    ...
    def __getitem__(self, idx):
        ...
        state = torch.tensor(sample["embeddings"][0]).float()   # original partial point cloud
        next_state = torch.tensor(sample["embeddings"][1]).float()      

        action = (torch.tensor(sample["action"])*1000).squeeze().float()

        sample = {"state": state, "action": action, "next_state": next_state}        
