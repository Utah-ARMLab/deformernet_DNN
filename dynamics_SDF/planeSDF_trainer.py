...
from architecture import PlaneSDF
from dataset_loader_planeSDF import PlaneSDFDataset
...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        percent_passed = sample["percent passed"].to(device)      
  
        
        optimizer.zero_grad()
        output = model(state, plane)

        loss = F.mse_loss(output, percent_passed)
        loss.backward()
...
def test(model, device, test_loader, epoch):
    ...
    with torch.no_grad():
        for sample in test_loader:
            ...
            test_loss += F.mse_loss(output, percent_passed, reduction='sum').item()
...
if __name__ == "__main__":
    ...
    dataset = PlaneSDFDataset(percentage = 1.0)
    ...
    weight_path = "/home/baothach/shape_servo_data/RL_shapeservo/box/weights_SDF_plane/run1/"
