...
class DensityNet(nn.Module):
    def __init__(self, hidden_unit = [16, 8]):
        ...
        self.mlp_bns.append(nn.BatchNorm2d(hidden_unit[0], momentum = 0.5))
        for i in range(1, len(hidden_unit)):
            self.mlp_convs.append(nn.Conv2d(hidden_unit[i - 1], hidden_unit[i], 1))
            self.mlp_bns.append(nn.BatchNorm2d(hidden_unit[i], momentum = 0.5))
        self.mlp_convs.append(nn.Conv2d(hidden_unit[-1], 1, 1))
        self.mlp_bns.append(nn.BatchNorm2d(1, momentum = 0.5))
...
class WeightNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, hidden_unit = [8, 8]):
        ...
        if hidden_unit is None or len(hidden_unit) == 0:
            ...
            self.mlp_bns.append(nn.BatchNorm2d(out_channel, momentum = 0.5))
        else:
