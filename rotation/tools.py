...
def normalize_5d_rotation(r5d):
    ...
    sin_cos_mag = torch.max(
        torch.sqrt(sin_cos.pow(2).sum(1)),
        torch.autograd.Variable(torch.DoubleTensor([1e-8]).cuda()),
    )  # batch
...
def rotation5d_to_matrix(r5d):
    ...
    matrix = torch.cat(
        (row1.view(-1, 1, 3), row2.view(-1, 1, 3), row3.view(-1, 1, 3)), 1
    )  # batch*3*3*seq_len
...
def proj_u_a(u, a):
    ...
    bottom = torch.max(
        torch.autograd.Variable(torch.zeros(batch).cuda()) + 1e-8, bottom
    )
...
def compute_quaternions_from_rotation_matrices(matrices):
    ...
    w = (
        torch.sqrt(1.0 + matrices[:, 0, 0] + matrices[:, 1, 1] + matrices[:, 2, 2])
        / 2.0
    )
    ...
    quats = torch.cat(
        (w.view(batch, 1), x.view(batch, 1), y.view(batch, 1), z.view(batch, 1)), 1
    )
