...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        loss_rot = model.compute_geodesic_loss(target_rot_mat, rot_mat) * 147 # make loss_pos = 3 * loss_rot
        ...
        if batch_idx % 10 == 0:
            ...
            if epoch == 0 and batch_idx ==0:
                logger.info("(Initial) loss pos: {}, loss rot: {}, ratio: {}".format(int(loss_pos.detach().cpu().numpy()), int(loss_rot.detach().cpu().numpy()), loss_rot.detach().cpu().numpy()/loss_pos.detach().cpu().numpy())) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_mp_input', default="True", type=str, help="use MP as an input to the NN")
    ...
    if use_mp_input:
        weight_path = f"/home/baothach/shape_servo_data/rotation_extension/multi_{args.obj_category}/weights/run2_w_rot_w_MP/"
    else:
        weight_path = f"/home/baothach/shape_servo_data/rotation_extension/multi_{args.obj_category}/weights/run2_w_rot_no_MP/"
    ...
    dataset = SingleBoxDataset(percentage = 1.0, use_mp_input=use_mp_input, dataset_path=dataset_path)
