...
for i in range(start_index, max_len_data):
    ...
    _, nearest_idxs_1 = neigh.kneighbors(np.array(list(data["mani_point_1"][0])).reshape(1, -1))
    mp_channel[nearest_idxs_1.flatten()] = 1

    _, nearest_idxs_2 = neigh.kneighbors(np.array(list(data["mani_point_2"][0])).reshape(1, -1))
