...
def save_pickle(point_cloud, can_mp_pos, label, data, idx, vis=False, gt_mp=None):
    ...
    processed_data = {"partial pcs": pcs, "label": label, \
                    "mani_point": mani_point, "obj_name": data["obj_name"]} 

    with open(os.path.join(data_processed_path, f"sample {idx}.pickle"), 'wb') as handle:
...
for i in range(start_index, max_len_data):
    ...
    mp_pos = np.array(list(data["mani_point"][0]))#np.array([-mani_point[0,3], -mani_point[1,3], mani_point[2,3] + ROBOT_Z_OFFSET])
