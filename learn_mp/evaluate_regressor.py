...
model = ManiPointNet2(model_type = 'regressor').to(device)
weight_path = "/home/baothach/shape_servo_data/manipulation_points/box/weights/regressor/run1"
...
num_candidates = 500
batch_size = 64
with open(os.path.join(data_recording_path, file_names[0]), 'rb') as handle:
    data_2 = pickle.load(handle)
# for i in [500, 515, 530, 545, 560, 585, 620]:
# for i in [525, 535]:
for i in [500, 515, 525, 530, 545, 560, 585, 620]:
# for i in list(np.random.randint(low=0, high=9379, size=6)):
    ...
    pc = down_sampling(data_2["partial pcs"][0])
    ...
    pcs_tensor = torch.from_numpy(pc).permute(1,0).unsqueeze(0).repeat(mp_candidates.shape[0],1,1)
    ...
    with torch.no_grad():
        outputs = []
        for batch_pc, batch_pc_goal in zip(torch.split(modified_pc_tensor, max(1,num_candidates//batch_size)), torch.split(pcs_goal_tensor, max(1,num_candidates//batch_size))):
        ...
        success_probs = output.cpu().detach().numpy()[:,0]
        success_probs = min(success_probs)/success_probs
        # print("success_prob:",success_probs)
        heats = np.array([[prob, 0, 0] for prob in success_probs])

        best_mp = mp_candidates[np.argmin(output.cpu().detach().numpy()[:,0])]
